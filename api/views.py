...
class UserView(View):
    ...
    def get(self, request, id_user, id_user_stalker):
        ...
        friends_one = list(FriendRequest.objects.filter(
            user_s_id=user).filter(status_id=accepted).annotate(idCard=('id'), idUser=F('user_r__id'), name=F('user_r__name'), email=F('user_r__email'), english_level_id=F('user_r__english_level_id'), id_firebase=F('user_r__id_firebase'), image=F('user_r__image')).values('idUser', 'name', 'email', 'english_level_id', 'id_firebase',  'image'))

        friends_two = list(FriendRequest.objects.filter(
            user_r_id=user).filter(status_id=accepted).annotate(idCard=('id'), idUser=F('user_s__id'), name=F('user_s__name'), email=F('user_s__email'), english_level_id=F('user_s__english_level_id'), id_firebase=F('user_s__id_firebase'), image=F('user_s__image')).values('idUser', 'name', 'email', 'english_level_id', 'id_firebase', 'image'))
