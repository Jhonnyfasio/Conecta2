        ...
        cards = list(CardPost.objects.annotate(isLike=Count(
            'like_card', filter=Q(like_card__status=True, like_card__user_id=user))).annotate(isSave=Count(
                'save_card', filter=Q(save_card__status=True, save_card__user_id=user))).annotate(countLike=Count(
                    'like_card', filter=Q(like_card__status=True))).values('user_id__name', 'content', 'category_id', 'user_id', 'isLike', 'isSave', 'countLike'))
...
class LikeView(View):
    ...
    def post(self, request):
        ...
        like = list(Like.objects.filter(
            card_id=card, user_id=user).values('id'))

        if len(like) == 1:
            data = {'message': 'LIKE', 'like': like[0].id}
        else:
